   ...
   def construct_profile(self):
      ...
      for i,m in enumerate(self.meshes):
         ...
         if m.material.type is 'semiconductor' :
            ...
            raise ValueError("Error, material type (%s) unknown!" %(material.type))
         x = y
   ...
   def calc_np(self):
      ...
      for m in self.meshes :
         ...
         if m.material.type is "semiconductor":
            ...
            raise ValueError("Error!! Material %s unkown!!" % m.material.type)
         i = j
      assert i == self.c_size
   def calc_Ef(self):
      i = 0
...
class mesh1D(object):
   ...
   def __init__ (self,dx,N,pos,material):
      ...
      if material.type == 'semiconductor' :
         ...
         raise ValueError("mesh2D() Error, material type (%s) unknown!"%(material.type))
   ...
   **********************************************************
   ...
   def add_mesh(self,N,pos,material) :
      if any([overlap(pos,N,m.pos,m.N) for m in self.meshes]) :
         raise RuntimeError("solver1D.add_mesh ERROR, mesh overlap detected!!")
   ...
   def add_contact(self,x,type=0):
      ...
      if any([m.pos<=x<=m.pos+m.N-1 for m in self.meshes]):
         raise ValueError("error! contact should not overlap with meshes")
   ...
   **********************************************************
   ...
   def add_mesh(self, N, pos=[0,0], material='Si') :
      ...
      if any([overlap(pos[0],N[0],m.pos[0],m.N[0])
          ...
          for m in self.meshes]):
         raise RuntimeError("solver2D.add_mesh ERROR, mesh overlap detected!!")
