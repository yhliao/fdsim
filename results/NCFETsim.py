...
def FEplusMOS(Qstack,Vstack,QMOS,VMOS,ID) :
   UNDEF = 100
   Vstack = np.interp(QMOS*1,Qstack,Vstack,left=UNDEF, right=UNDEF)
   valid_idx = (Vstack!=UNDEF)
   QMOS   = QMOS[valid_idx]
   VNCFET = VMOS[valid_idx] + Vstack[valid_idx]
   ID     = ID[valid_idx]
   return QMOS, VNCFET, ID, Vstack[valid_idx]

def show(show=True,save=True):
   fig = plt.figure(figsize=(20,10))
   ### plot the Q-V relations
   plt.subplot(1,3,1)
   plt.plot(V_NCFET,Q_MOS, label='NCFET')
   plt.plot(vv,Q_MOS, label='stack')
...
Q_MOS, V_NCFET, I_D,vv = FEplusMOS(Qstack,Vstack,QMOS,VMOS,ID)
calcSS(VMOS,ID)
calcSS(V_NCFET,I_D)
show(show=False)
